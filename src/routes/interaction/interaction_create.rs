    ...
    match verify_sign {
        true => {
            ...
            if type_int == HTTP_INTERACTION_CONFIRMATION_BOT {
                warp::reply::with_status(warp::reply::json(&json!({ "type": 1 }).as_object_mut()), warp::http::StatusCode::OK);
            }
            if type_int == INTERACTION_COMMAND {
                if json.get("application_id").is_none() {
                    warp::reply::with_status(warp::reply::json(&json!({ "type": 5, "message_err": "API cannot accept this metadata because application was not included! Please resend again." }).as_object_mut()), warp::http::StatusCode::NOT_ACCEPTABLE);
                }
                for (id, client) in clients.read().await.iter() {
                    if json.get("application_id").unwrap() == id {
                        if let Err(_disconnected) =  client.tx.send(Message::binary(convert_to_binary(&json!(json)))) {
                                ...
                                warp::reply::with_status(warp::reply::json(&json!({ "type": 4, "data": {
                                "embeds": [
                                    {
                        ...
                        warp::reply::with_status(warp::reply::json(&json!({ "type": 5 }).as_object_mut()), warp::http::StatusCode::OK);
